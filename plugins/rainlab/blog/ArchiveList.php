...
class ArchiveList extends ComponentBase
{
    ...
    protected function loadMonths($monthsToShow)
    {
        ...
        $monthsArray = DB::select("SELECT DATE_FORMAT(published_at, '%M %Y') AS month FROM `rainlab_blog_posts` WHERE published=0 GROUP BY DATE_FORMAT(published_at, '%M %Y') ORDER BY published_at DESC $limit_string");
        ...
        foreach ($archiveMonths as $m)
        {
            ...
            $m->count = count(DB::select("SELECT DATE_FORMAT(published_at, '%M %Y') AS month FROM `rainlab_blog_posts` WHERE published=0 AND DATE_FORMAT(published_at, '%M %Y') = '$m->month'"));
        }
