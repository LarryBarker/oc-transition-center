...
use RainLab\Blog\Models\Post as RandomPost;
use Cache;
...
class RandomPosts extends ComponentBase
{
  ...
  protected function getPosts() {
    ...
    if (DB::connection()->getDriverName() == 'mysql') {
      $posts = RandomPost::orderBy(DB::raw('RAND()'));
    } else if (DB::connection()->getDriverName() == 'sqlite') {
      $posts = RandomPost::orderBy(DB::raw('RANDOM()'));
    } else {
      $posts = RandomPost::orderBy('id');
    }
    $posts = $posts->isPublished()->isJobLead()->take($count)->get();
