<?php namespace Wwrf\MobilePlugin\Providers;
...
class DefaultProvider extends ProviderBase
{
    ...
    public function signin()
    {
        ...
        $rules['instance_id'] = 'required|max:16|string|exists:wwrf_mobile_installs,instance_id';
    ...
    public function register()
    {
        try {
            ...
            $rules = [
                ...
                'instance_id' => 'required|max:16|string|exists:wwrf_mobile_installs,instance_id',
                'package'     => 'required|regex:/^[a-z0-9]*(\.[a-z0-9]+)+[0-9a-z]$/|exists:wwrf_mobile_variants|registration_enabled'
            ];
            ...
            Validator::extend('registration_enabled', function ($attribute, $value, $parameters, $validator)
            {
                if($variant = Variant::where('package', '=', $value) -> first()) {
                  if($variant -> disable_registration)
                    throw new ValidationException(['package' => trans('wwrf.mobileplugin::lang.variants.registration_disabled')]);
                  return true;
                }
                return false;
            });
