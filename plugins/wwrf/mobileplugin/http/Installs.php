...
class Installs extends Controller
{
    ...
    public function store()
    {
      ...
      $beforeSaveResponses = Event::fire('wwrf.mobileplugin.beforeSave', [$instance_id, $package]);
      foreach($beforeSaveResponses as $beforeSaveResponse)
        {
          if(!$beforeSaveResponse instanceof \Illuminate\Http\JsonResponse)
            throw new ApplicationException('The event wwrf.mobileplugin.beforeSave can only return JsonResponse');
      ...
      */
      ...
      if($install -> save()) {
        ...
        $afterSaveResponses = Event::fire('wwrf.mobileplugin.afterSave', [$install]);
        foreach($afterSaveResponses as $afterSaveResponse)
          {
            if(!$afterSaveResponse instanceof \Illuminate\Http\JsonResponse)
              throw new ApplicationException('The event wwrf.mobileplugin.afterSave can only return JsonResponse');
